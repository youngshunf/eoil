...
(function($, owner) {
	...
	owner.getAuthUrl=function(url){
		var userInfo=owner.getUserInfo();
		if(userInfo){
			return url+'?access-token='+userInfo.access_token;
		}
		return url;
	}
...
function checkMobile(mobile){
   ...
   function updateLoc(){
   		plus.geolocation.getCurrentPosition(function(position){
			...
			mui.ajax({
				type:"post",
				url:app.getAuthUrl(config.uploadLocationUrl),
				data:{
					data:data
				},
				success:function(rs){
				...
				error:function(e){
					console.log('上传位置失败'+JSON.stringify(e));
				}
			});
